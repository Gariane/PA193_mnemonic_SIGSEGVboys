...
class Mnemonic {
   ...
   public:
     ...
     * BIP39::Mnemonic::fromEntropy::Entropy or left empty, so constructor call
     * is not ambiguous
     * @param entropy hexadecimal string to use as initial entropy, must be
     * 32-64 bytes long and number of bytes must be multiple of 8
     ...
     * @throws std::invalid_argument if phrase contains word not present in
     * dict, or if phrase checksum is not valid
