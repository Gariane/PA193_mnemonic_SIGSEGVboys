 ...
 * @brief BIP39 mnemonic phrase and seed generator and verifier for PA193 group
 * project
 */
#ifndef MNEMONIC_HPP
#define MNEMONIC_HPP
...
/**
 * @brief Class provides functionality to calculate BIP39 mnemonic phrase and
 * seed
 */
class Mnemonic {
public:
...
public:
  /**
   * @brief Generates mnemonic sentence and seed from supplied entropy,
   * passphrase and dictionary NOTE: last parameter must be
   * BIP39::Mnemonic::fromEntropy::Entropy or left empty, so constructor call is
   * not ambiguous
   * @param entropy hexadecimal string to use as initial entropy, must be 32-64
   * bytes long and number of bytes must be multiple of 8
   * @param passphrase passphrase to use, no constraints
   * @param dict valid BIP39::Dictionary object used to generate mnemonic
   * sentence
   * @throws std::invalid_argument if initial entropy is not valid
   * @throws std::runtime_error if checksum, seed or mnemonic sentence
   * generation failed
   */
  Mnemonic(std::string entropy, const std::string &passphrase,
           const BIP39::Dictionary &dict,
           fromEntropy /* unused */ = fromEntropy::Entropy);

  /**
   * @brief Generates initial entropy and seed from mnemonic phrase and
   * passphrase and dictionary NOTE: last parameter must be
   * BIP39::Mnemonic::fromPhrase::Phrase, so constructor call is not ambiguous
   ...
   * @throws std::invalid_argument if phrase contains word not present in dict,
   * or if phrase checksum is not valid
...
private:
  ...
  static std::string generateSeed(const std::string &mnemonic,
                                  const std::string &passphrase);
  static uint8_t calculateChecksum(const std::vector<uint8_t> &entropy);
  static std::vector<uint8_t> getBytesFromPhrase(const std::string &phrase,
