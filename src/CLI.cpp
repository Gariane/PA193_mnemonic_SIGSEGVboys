...
private:
  ...
  bool checkPhraseThrow(const std::stringstream &phraseBuffer,
    ...
    try {
      return Mnemonic::checkPhraseSeedPair(phraseBuffer.str(), seedBuffer.str(),
                                           password, dict);
    } catch (std::invalid_argument &error) {
      std::cerr << error.what() << std::endl;
      std::exit(1);
...
public:
    ...
    while ((ch = getopt_long(argc, argv, "hd:p:c:g:r:", long_options,
      ...
      case 'c':
        ...
        if (optind < argc && argv[optind][0] != '-') {
          ...
          std::cerr
              << "Wrong parameter specified, use -h or --help for usage info";
          printHelp();
          std::exit(1);
        }
        break;
  ...
  static void printHelp() {
    ...
    std::cout << "-h --help                                 Prints help"
              << std::endl;
    std::cout << "-d --dictionary <path>                    Sets path to file "
                 "with dictionary"
              << std::endl;
    std::cout << "-p --password <password>                  Sets password"
              << std::endl;
    std::cout << "-c --check <phrase> <seed>                Check = phrase & "
                 "seed -> OK/NOK"
              << std::endl;
    std::cout << "-g --generate <entropy>                   Generate = entropy "
                 "-> seed and phrase"
              << std::endl;
    std::cout << "-r --reverse <phrase>                     Reverse = phrase "
                 "-> entropy and seed"
              << std::endl;
  }
};
